...
export const EMAIL_TEMPLATES = {
  ...
  REQUEST_AWAITING_L1: {
    subject: "Approval needed (Level 1) - Request {requestId}",
    template: (data, branding) => {
      const item = data.itemsSummary || data.assetName || "Asset request";
      const content = `
        <h2>🔔 New Request Awaiting Level 1 Approval</h2>
        <p>Dear Approver,</p>
        <p>A new asset request has been submitted and needs your <strong>first-level (L1)</strong> review.</p>

        <div class="info-box">
          <table class="details-table">
            <tr><th>Requester:</th><td>${data.requesterName}</td></tr>
            <tr><th>Item(s):</th><td>${item}</td></tr>
            <tr><th>Request ID:</th><td>#${data.requestId}</td></tr>
        ...
        <p>Once you approve, it will be escalated to a superadmin for final approval.</p>
        <a href="${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/admin/requests" class="button">
          Review Request
        </a>
        <p>Best regards,<br>${EMAIL_SIGNATURE}</p>
  ...
  REQUEST_AWAITING_L2: {
    subject: "Final approval needed - Request {requestId}",
    template: (data, branding) => {
      const item = data.itemsSummary || data.assetName || "Asset request";
      const content = `
        <h2>🛡️ Request Awaiting Final Approval</h2>
        <p>Dear Superadmin,</p>
        <p>A request has passed <strong>Level 1</strong> approval${data.l1ApproverName ? ` (by ${data.l1ApproverName})` : ''} and now needs your <strong>final (L2)</strong> approval.</p>
  ...
  REQUEST_L2_DECISION_CONFIRM: {
    ...
    template: (data, branding) => {
      ...
      const issued = data.decision === "approved_and_issued";
      const content = `
        <h2>✅ Final approval recorded</h2>
        <p>Dear Superadmin,</p>
        <p>
          ${
            data.approverName
              ? `<strong>${data.approverName}</strong> has given final approval`
              : "Final approval has been given"
          }
          for the request below${issued ? " and consumables were issued" : ""}.
        </p>
