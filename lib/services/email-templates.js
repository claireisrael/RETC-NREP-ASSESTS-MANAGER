...
export const EMAIL_TEMPLATES = {
  ...
  RETURN_REPORT_SUBMITTED: {
    subject: "Return report submitted — {itemName}",
    template: (data, branding) => {
      const appUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
      const content = `
        <h2>Return report to review</h2>
        <p>Dear Admin,</p>
        <p><strong>${data.requesterName || "A staff member"}</strong> submitted a digital return report.</p>
        <div class="info-box">
          <table class="details-table">
            <tr><th>Item:</th><td>${data.itemName || "—"}${data.assetTag ? ` (${data.assetTag})` : ""}</td></tr>
            <tr><th>Condition:</th><td>${data.condition || "—"}</td></tr>
            ${data.reason ? `<tr><th>Reason:</th><td>${data.reason}</td></tr>` : ""}
          </table>
        </div>
        <a href="${appUrl}/admin/returns" class="button">Review return reports</a>
        ${emailSignOff()}
      `;
      return baseTemplate(content, branding);
    },
  ...
  RETURN_REPORT_L2_AWARENESS: {
    subject: "Return confirmed — {itemName}",
    template: (data, branding) => {
      const content = `
        <h2>Return confirmed (awareness)</h2>
        <p>Dear Approver,</p>
        <p>An asset admin confirmed a return. No action is required from you.</p>
        <div class="info-box">
          <table class="details-table">
            <tr><th>Item:</th><td>${data.itemName || "—"}</td></tr>
            <tr><th>Condition:</th><td>${data.condition || "—"}</td></tr>
            <tr><th>Confirmed by:</th><td>${data.adminName || "Admin"}</td></tr>
          </table>
        </div>
        ${emailSignOff()}
      `;
  ...
  RETURN_REPORT_NEEDS_L2: {
    subject: "Action required: return exception — {itemName}",
    template: (data, branding) => {
      const appUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
      const content = `
        <h2>Return exception — acknowledge required</h2>
        <p>Dear Approver,</p>
        <p>A return was confirmed in poor or lost condition and needs your acknowledgement.</p>
        <div class="warning-box">
          <table class="details-table">
            <tr><th>Item:</th><td>${data.itemName || "—"}</td></tr>
            <tr><th>Condition:</th><td>${data.condition || "—"}</td></tr>
            <tr><th>Admin:</th><td>${data.adminName || "Admin"}</td></tr>
            ${data.reason ? `<tr><th>Reason:</th><td>${data.reason}</td></tr>` : ""}
            ${data.recommendation ? `<tr><th>Recommendation:</th><td>${data.recommendation}</td></tr>` : ""}
          </table>
        </div>
        <a href="${appUrl}/admin/returns" class="button">Acknowledge write-off</a>
        ${emailSignOff()}
      `;
      return baseTemplate(content, branding);
    },
  ...
  CATALOG_AVAILABILITY_PENDING: {
    subject: "Confirm availability — {itemName}",
    template: (data, branding) => {
      const appUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
      const content = `
        <h2>Catalog item awaiting availability confirmation</h2>
        <p>Dear Approver,</p>
        <p><strong>${data.createdByName || "An admin"}</strong> added a catalog item that needs your confirmation before it can be requested.</p>
        <div class="info-box">
          <table class="details-table">
            <tr><th>Item:</th><td>${data.itemName || "—"}${data.assetTag ? ` (${data.assetTag})` : ""}</td></tr>
            <tr><th>Type:</th><td>${data.itemType || "—"}</td></tr>
          </table>
        </div>
        <a href="${appUrl}/admin/availability" class="button">Confirm availability</a>
        ${emailSignOff()}
      `;
      return baseTemplate(content, branding);
    },
  ...
  CATALOG_AVAILABILITY_DECIDED: {
    subject: (data) =>
      `Availability ${data.confirmed ? "confirmed" : "rejected"} — ${data.itemName || "item"}`,
    template: (data, branding) => {
      const decided = data.confirmed ? "confirmed" : "rejected";
      const content = `
        <h2>Catalog availability ${decided}</h2>
        <p>Dear Admin,</p>
        <p><strong>${data.decidedByName || "L2"}</strong> ${decided} availability for a catalog item.</p>
        <div class="info-box">
          <table class="details-table">
            <tr><th>Item:</th><td>${data.itemName || "—"}</td></tr>
            <tr><th>Decision:</th><td>${decided}</td></tr>
            ${data.note ? `<tr><th>Note:</th><td>${data.note}</td></tr>` : ""}
          </table>
        </div>
        ${emailSignOff()}
      `;
