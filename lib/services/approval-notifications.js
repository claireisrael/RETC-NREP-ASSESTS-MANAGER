...
// L1 approved -> notify the assigned L2 superadmin (+ requester progressed).
export async function notifyL1Approved(request, requester, l1Approver, items = []) {
  ...
  try {
    let l2Emails = [];
    const assignedId = request?.assignedL2StaffId;

    if (assignedId) {
      try {
        const { staffService } = await import("../appwrite/provider.js");
        const assigned = await staffService.get(assignedId);
        if (assigned?.email) {
          l2Emails = [String(assigned.email).toLowerCase().trim()];
        }
      } catch (lookupErr) {
        console.warn(
          "notifyL1Approved: assigned L2 lookup failed:",
          lookupErr
        );
      }
    }

    // Fallback only when no assignee was set (legacy requests).
